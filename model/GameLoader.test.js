...
QUnit.test("load()", (assert) => {
  ...
  GameLoader.load(store).then(() => {
    ...
    assert.equal(Object.keys(gameToUsers).length, 702, "gameToUsers length");
    ...
    assert.equal(Object.keys(gameToDetail).length, 863, "gameToDetail length");
    done();
  });
});
...
QUnit.test("loadCollections()", (assert) => {
  ...
  GameLoader.loadCollections(store).then(() => {
    ...
    assert.ok(gameToUsers);
    assert.equal(Object.keys(gameToUsers).length, 702);
    done();
  });
});
...
QUnit.test("loadWishlists()", (assert) => {
  ...
  GameLoader.loadWishlists(store).then(() => {
    ...
    assert.equal(Object.keys(wishToUsers).length, 31);
    done();
  });
});
